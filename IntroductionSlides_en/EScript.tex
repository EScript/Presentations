...
\begin{itemize}
	\item To pass data to loaded or evalated code, static variables can be injected.
	\item The variables are passed as additional Map-parameter to \lstinline!eval! or \lstinline!load!.
outln(
	"outln('Value of myVar:',myVar); ",
	{	$myVar : 42 }
);
\end{itemize}
\end{frame}
...
\begin{itemize}
		...
		\item Goals: 
		\begin{itmeize}
			\item Provide helper functions and data types
			\item Support project structuring using \textbf{modules}.
		\end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Init the StdLib}
\begin{itemize}
		\item (quick and dirty) Load the complete StdLib into the global Std-namespace:
	%		static module = load( IO.dirname(args[0])+ "/Std/module.escript"); 
\begin{lstlisting}
		load("./Std/complete.escript");
		// Std - contains all components of the StdLib.
		// e.g. Std.MultiProcedure
\end{lstlisting}
